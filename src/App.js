...
const RESUME_DATA = {
  ...
  yearsExperience: "20+",
  whyHireMe: [
    "Conducted comprehensive security and best practice assessments of cloud-native tools across AWS and Azure for the U.S. Department of Veterans Affairs over 1.5 years",
    "6 years as Senior Consultant with progressive responsibilities in solution engineering, data center-to-cloud migrations, and team mentoring across enterprise clients",
    "Deep expertise in Azure Government and federal compliance frameworks (NIST 800-53, CMMC, FedRAMP) with hands-on ATO readiness experience",
    "Active certifications (AZ-900, SC-900, SSCP, GIAC) combined with Microsoft Sentinel, Defender for Cloud, and Zero Trust Architecture implementation experience"
  ],
  experience: [
    {
      company: "Maveris",
  ...
  competencies: [
    ...
    "NIST 800-53 / CIS v3 / CMMC / FedRAMP",
    "Cloud Security Posture Management",
    "Incident Detection & Response",
    "Infrastructure as Code",
    "PowerShell Automation & Compliance",
    "Azure Policy & Landing Zones",
    "Continuous Monitoring & ATO Readiness",
  ],
};
          ...
          {compareMode && (
            <div style={{marginTop:14}}>
              ...
              <div style={{display:"flex",gap:8,marginBottom:8}}>
                ...
                <button onClick={()=>{const name=document.getElementById("candName").value.trim()||`Candidate ${candidateResumes.length+1}`;const text=prompt("Paste candidate's resume text:");if(text) setCandidateResumes([...candidateResumes,{name,text}]);document.getElementById("candName").value="";}} style={{padding:"8px 16px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#64a0ff,#a070ff)",color:"#fff",fontSize:12,fontWeight:500,cursor:"pointer"}}>+ Add</button>
              </div>
...
EXECUTIVE SNAPSHOT
Clearance: ${RESUME_DATA.clearance}
Citizenship: ${RESUME_DATA.citizenship}
Availability: ${RESUME_DATA.availability}
Work Preference: ${RESUME_DATA.workPreference}
...
WHY HIRE ME
${RESUME_DATA.whyHireMe.map(item=>`• ${item}`).join("\n")}
...
function generateHiringManagerPackage(jobFitResult) {
...
Availability:     ${RESUME_DATA.availability}
Work Preference:  ${RESUME_DATA.workPreference}
Experience:       ${RESUME_DATA.yearsExperience} years in IT & Cybersecurity

${jobFitResult ? `
CANDIDATE FIT ANALYSIS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Overall Match: ${jobFitResult.overallScore}% — ${jobFitResult.overallScore>=80?"EXCELLENT FIT":jobFitResult.overallScore>=60?"GOOD FIT":jobFitResult.overallScore>=40?"PARTIAL FIT":"LOW FIT"}

${jobFitResult.summary}

Key Strengths:
${jobFitResult.matches.map(m=>`  ✓ ${m}`).join("\n")}

Skill Gaps to Address:
${jobFitResult.gaps.map(g=>`  ⚠ ${g}`).join("\n")}
`:''}

TOP 4 REASONS TO HIRE THIS CANDIDATE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
${RESUME_DATA.whyHireMe.map((item,i)=>`${i+1}. ${item}`).join("\n\n")}
...
═══════════════════════════════════════════════════════════════
Full resume attached separately.
Generated on ${new Date().toLocaleDateString()}
`;
  const blob = new Blob([packageContent],{type:"text/plain"});
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href=url; a.download=`${RESUME_DATA.name.replace(/\s/g,"_")}_Hiring_Manager_Package.txt`;
  a.click(); URL.revokeObjectURL(url);
}
...
export default function App() {
  ...
  const [lastJobFitResult, setLastJobFitResult] = useState(null);
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:800,margin:"0 auto",padding:"50px 24px 140px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:28,animation:"slideUp 0.7s ease"}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:10,marginTop:20,flexWrap:"wrap"}}>
            ...
            <button onClick={()=>generateHiringManagerPackage(lastJobFitResult)} style={{padding:"9px 18px",borderRadius:10,border:"1px solid rgba(200,168,74,0.3)",background:"rgba(200,168,74,0.08)",color:"#c8a84a",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",display:"flex",alignItems:"center",gap:6,transition:"all 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(200,168,74,0.15)";e.currentTarget.style.borderColor="rgba(200,168,74,0.5)";}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(200,168,74,0.08)";e.currentTarget.style.borderColor="rgba(200,168,74,0.3)";}}>
            ...
            <button onClick={()=>(window.location.href=`mailto:${RESUME_DATA.email}?subject=Schedule a Call&body=Hi Jeff, I'd like to schedule a call to discuss opportunities.`)} style={{padding:"9px 18px",borderRadius:10,border:"1px solid rgba(160,112,255,0.3)",background:"rgba(160,112,255,0.08)",color:"#a070ff",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",display:"flex",alignItems:"center",gap:6,transition:"all 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(160,112,255,0.15)";e.currentTarget.style.borderColor="rgba(160,112,255,0.5)";}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(160,112,255,0.08)";e.currentTarget.style.borderColor="rgba(160,112,255,0.3)";}}>
        ...
        {/* ══════ EXECUTIVE DASHBOARD ══════ */}
        <div style={{marginBottom:28,animation:"fadeInUp 0.5s ease 0.1s both"}}>
          <div style={{background:"linear-gradient(135deg, rgba(100,160,255,0.08) 0%, rgba(160,112,255,0.08) 100%)",border:"1px solid rgba(100,160,255,0.2)",borderRadius:14,padding:"18px 22px"}}>
            <p style={{margin:"0 0 12px",fontSize:11,color:"#64a0ff",textTransform:"uppercase",letterSpacing:"0.1em",fontWeight:600}}>📊 Executive Dashboard</p>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12}}>
              {[
                {label:"Clearance",value:RESUME_DATA.clearance,icon:"🔒"},
                {label:"Citizenship",value:RESUME_DATA.citizenship,icon:"🇺🇸"},
                {label:"Availability",value:RESUME_DATA.availability,icon:"⏱️"},
                {label:"Work Preference",value:RESUME_DATA.workPreference,icon:"🏠"},
                {label:"Experience",value:RESUME_DATA.yearsExperience+" years",icon:"📅"},
              ].map((item,i)=>(
                <div key={i} style={{textAlign:"center"}}>
                  <p style={{margin:0,fontSize:20}}>{item.icon}</p>
                  <p style={{margin:"6px 0 2px",fontSize:14,fontWeight:700,color:"#e8ecf0"}}>{item.value}</p>
                  <p style={{margin:0,fontSize:10,color:"#6b7a8d",textTransform:"uppercase",letterSpacing:"0.05em"}}>{item.label}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ══════ WHY HIRE ME ══════ */}
        <div style={{marginBottom:28,animation:"fadeInUp 0.5s ease 0.14s both"}}>
          <div style={{background:"linear-gradient(135deg, rgba(76,175,122,0.08) 0%, rgba(100,160,255,0.08) 100%)",border:"1px solid rgba(76,175,122,0.2)",borderRadius:14,padding:"18px 22px"}}>
            <p style={{margin:"0 0 14px",fontSize:11,color:"#4caf7a",textTransform:"uppercase",letterSpacing:"0.1em",fontWeight:600}}>✨ Why Hire Me</p>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {RESUME_DATA.whyHireMe.map((item,i)=>(
                <div key={i} style={{display:"flex",gap:10}}>
                  <span style={{fontSize:16,flexShrink:0}}>✓</span>
                  <p style={{margin:0,fontSize:12.5,color:"#c8d0dc",lineHeight:1.6}}>{item}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* SUMMARY */}
        <div style={{background:"rgba(255,255,255,.035)",border:"1px solid rgba(255,255,255,.07)",borderRadius:14,padding:"18px 22px",marginBottom:28,animation:"fadeInUp 0.6s ease 0.18s both"}}>
          <p style={{fontSize:13,color:"#8a95a8",lineHeight:1.8}}>{RESUME_DATA.summary}</p>
        </div>

        {/* COMPETENCIES */}
        <div style={{marginBottom:28,animation:"fadeInUp 0.6s ease 0.22s both"}}>
        ...
        {/* TABS */}
        <div style={{display:"flex",gap:5,marginBottom:20,flexWrap:"wrap",animation:"fadeInUp 0.5s ease 0.26s both"}}>
